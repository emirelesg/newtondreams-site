...
var originalPlot = new p$.Plot({
  limit: 1000,
  drawInvisiblePoints: true,
  color: p$.BOX_COLORS.RED.BACKGROUND
});
var controls = {};
var labels = {};
...
function reset() {
  ...
  var formulaWidth =
    (texts[texts.length - 1].x + texts[texts.length - 1].width + TEXT_OFFSET) *
    w.scaleX.toPx;
...
function draw() {
  ...
  for (var x = xMin; x <= xMax; x += 0.1) {
    ...
    plot.addPoint(
      x,
      controls.a.value *
        Math.pow(controls.b.value * (x + controls.c.value), 2) +
        controls.d.value
    );
  }
}
...
function drawFormula() {
  ...
  formula.save();
  formula.translate(
    (results.position.x + results.padding.top - w.axis.position.x) *
      w.scaleX.toUnits,
    (labels.formula.position.y +
      results.position.y +
