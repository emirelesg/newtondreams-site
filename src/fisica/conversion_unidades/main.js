...
function setupControls() {
  ...
  controls.valueA = new p$.dom.Input('value_a', aToB, {
    isNumber: true,
    default: 0
  });
  controls.valueB = new p$.dom.Input('value_b', bToA, {
    isNumber: true,
    default: 0
  });
...
function drawScene() {
  ...
  s.text(input, x, y);
  x += inputW + SPACE;
  drawMultiplication(x, y);
  x += SPACE + width / 2;
  drawFraction(x, y, width, numStr, denStr);
  x += SPACE + width / 2;
  drawEquals(x, y);
  x += SPACE;
  s.font.set({
    size: TEXT_SIZE,
    weight: 'bold',
    align: wrap ? 'center' : 'left'
  });
  s.text(output, wrap ? 0 : x, wrap ? -WRAP_Y : y);
