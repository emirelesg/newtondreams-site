...
function setup() {
  ...
  // Configure labels.
  p1.period_label = results.addLabel(120, 14, {
    name: 'Periodo 1',
    units: 's',
    labelWidth: 80,
    decPlaces: 2
  ...
  p2.period_label = results.addLabel(120, 14, {
    name: 'Periodo 2',
    units: 's',
    labelWidth: 80,
    decPlaces: 2
...
function reset() {
  ...
  p1.setInitialConditions(
    controls.lenght1.value,
    controls.angle.value * p$.DEG_TO_RAD
  );
  p1.update(0);
  p2.setInitialConditions(
    controls.length2.value,
    controls.angle.value * p$.DEG_TO_RAD
  );
  p2.update(0);
...
function drawScene() {
  ...
  scene.moveTo(0, 0);
  scene.lineTo(
    10 * Math.sin(MAX_ANGLE * p$.DEG_TO_RAD),
    -10 * Math.cos(MAX_ANGLE * p$.DEG_TO_RAD)
  );
  scene.lineTo(
    10 * Math.sin(-MAX_ANGLE * p$.DEG_TO_RAD),
    -10 * Math.cos(-MAX_ANGLE * p$.DEG_TO_RAD)
...
function draw() {
  ...
  if (started) {
    ...
    var maxDisplayTime =
      (graph.axis.width - graph.axis.position.x) * graph.scaleX.toUnits;
    if (t > maxDisplayTime) {
