...
function setup() {
  ...
  // Configure labels.
  labels.vel = results.addLabel(150, 14, {
    name: 'Velocidad: ',
    units: 'm/s',
    labelWidth: 100,
    decPlaces: 2
  ...
  labels.pressure = results.addLabel(150, 14, {
    name: 'Presión: ',
    units: 'atm',
    labelWidth: 100,
    decPlaces: 2
...
function setupControls() {
  ...
  controls.temperature = new p$.Slider({
    id: 'temperature',
    start: 300,
    min: 0,
    max: 1000,
...
function reset() {
  ...
  box_l_target =
    Math.pow(controls.volume.value * p$.CM3_TO_M3, 1 / 3) * p$.M_TO_CM;

  // Calculate the maximum possible speed for a particle.
  max_possible_speed = p$.utils.round(
    Math.sqrt((3 * p$.BOLTZMANN * controls.temperature.value) / mass_argon),
    0
  );
...
function draw() {
  ...
  function checkCollision(p1, p2) {
    ...
    if (d <= p1.r + p2.r) {
      ...
      var new_v1n =
        (v1n * (p1.mass - p2.mass) + 2 * p2.mass * v2n) / (p1.mass + p2.mass);
      var new_v2n =
        (v2n * (p2.mass - p1.mass) + 2 * p1.mass * v1n) / (p1.mass + p2.mass);
  ...
  if (Math.abs(box_l_diff) < 0.01) {
    // If the difference is small update directly the target.
    box_l = box_l_target;
  } else if (box_l_diff > 0) {
    // Target is larger.
    box_l += 0.01;
  } else if (box_l_diff < 0) {
    // Target is smaller.
  ...
  var pressure =
    (mass_argon * amount * max_possible_speed * max_possible_speed) /
    (3 * box_v);
