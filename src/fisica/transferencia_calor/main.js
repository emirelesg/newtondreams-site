...
var MIN_TEMP = -20,
  MAX_TEMP = 300; // Define the temperature range.
...
var MATERIALS = {
  // Defines the K constants. The keys must match the values of the Radio buttons.
  Otro: -1,
  Madera: 0.1,
  Concreto: 0.8,
  Acero: 50.2
...
var barWidth = 0; // Dimensions of the transfer bar.
var barLength = 0;
var arrowBodyW = 0;
...
function setup() {
  ...
  labels.heatTransfer = results.addLabel(130, 14, {
    name: 'Corriente',
    units: 'W',
    labelWidth: BOX_LABEL_WIDTH,
    decPlaces: 1
  });
...
function setupControls() {
  // Configure sliders.
  controls.barLength = new p$.Slider({
    id: 'bar_length',
    start: 2,
    min: 0.5,
    max: 3,
...
function reset() {
  ...
  heatTransfer =
    (controls.k.value *
      controls.barArea.value *
      (controls.t1.value - controls.t2.value)) /
    controls.barLength.value;
...
function draw3dRect(x, y, width, height, color, color2) {
  ...
  if (color2 !== undefined) {
    ...
    if (controls.vertical.value) {
      ...
      cFront.addColorStop(0, shadeColor(color, 75));
      cFront.addColorStop(1, shadeColor(color2, 75));
      cFrontBorder.addColorStop(0, shadeColor(color, 50));
      cFrontBorder.addColorStop(1, shadeColor(color2, 50));

      // Right gradients.
      cRight = w.ctx.createLinearGradient(0, 0, 0, barHeightPx);
      cRightBorder = cFrontBorder;
      cRight.addColorStop(0, shadeColor(color, 80));
      cRight.addColorStop(1, shadeColor(color2, 80));
      ...
      cTop.addColorStop(0, shadeColor(color, 85));
      cTop.addColorStop(1, shadeColor(color2, 85));
...
function drawSimulation() {
  ...
  function drawHeatArrow() {
    if (heatTransfer !== 0) {
      ...
      // Draw the arrow tip.
      if (arrowBodyW > 0) {
        sim.equilateralTriangle(
          arrowBodyW + ARROW_HEAD_H / 2,
          0,
          ARROW_HEAD_L,
          0
        );
