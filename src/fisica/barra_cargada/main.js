...
var particle = new p$.Ball(0.25, {
  color: p$.COLORS.PURPLE,
  topmostOnDrag: false,
  onMouseMove: particleMoved
});
var bar = new p$.Shape(drawBar, {});
var field = new p$.Vector({
  scale: 0.001,
  components: true,
  color: p$.COLORS.PURPLE
});
...
function setup() {
  ...
  var LABEL_WIDTH = 60;
  labels.e = results.addLabel(130, 14, {
    name: 'Campo',
    units: 'N/C',
  ...
  labels.angle = results.addLabel(130, 14, {
    name: p$.SYMBOL.THETA,
    units: '°',
    labelWidth: LABEL_WIDTH,
    decPlaces: 0
...
function particleMoved() {
  ...
  is_inside =
    Math.abs(w.mouse.rx) < BAR_WIDTH / 2 + particle.r &&
    w.mouse.ry < BAR_HEIGHT + particle.r &&
    w.mouse.ry > -particle.r / 2;
...
function draw() {
  ...
  var ex1 = k * lambda * xp * (yp / (xp_sq * r)) || 0;
  var ex2 =
    k *
      lambda *
      xp *
      ((l - yp) / (xp_sq * Math.sqrt(l * l - 2 * l * yp + xp_sq + yp_sq))) || 0;
  var ex = ex1 + ex2;
  ...
  if (xp != 0) {
    ...
    ey2 =
      k *
        lambda *
        (1 / Math.abs(xp) -
          1 / Math.sqrt(l * l - 2 * l * yp + xp_sq + yp_sq)) || 0;
    ey = ey1 - ey2;
  } else {
