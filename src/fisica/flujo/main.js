...
function setup() {
  ...
  labels.v1 = results.addLabel(130, 14, {
    name: 'V1',
    units: 'm/s',
    decPlaces: 2,
    labelWidth: 60
  ...
  labels.v2 = results.addLabel(130, 14, {
    name: 'V2',
    units: 'm/s',
    decPlaces: 2,
    labelWidth: 60
  ...
  labels.p_diff = results.addLabel(130, 14, {
    name: 'P2 - P1',
    units: 'Pa',
    usePrefixes: true,
    decPlaces: 1,
...
function draw() {
  ...
  for (var i = 0; i < points.length; i++) {
    ...
    if (points[i].x < JOINT_START) {
      ...
      v =
        v1 +
        ((v2 - v1) / (JOINT_END - JOINT_START)) * (points[i].x - JOINT_START);
      h =
        h1 +
        ((h2 - h1) / (JOINT_END - JOINT_START)) * (points[i].x - JOINT_START);
    } else {
      v = v2;
      h = h2;
    }
...
function resize() {
  if (w.width < 400) {
    w.scaleX.set(50, 3, 'm');
    w.axis.setPosition(
      Math.floor(w.width / 2 - (50 * LENGTH) / 6),
      w.height / 2 + 20
    );
  } else {
    w.scaleX.set(50, 2, 'm');
    w.axis.setPosition(
      Math.floor(w.width / 2 - (50 * LENGTH) / 4),
      w.height / 2 + 20
    );
  }
