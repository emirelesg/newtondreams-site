...
var equilibrium_point = 0; // Point around which the mass moves.
...
function setup() {
  ...
  damping_plot_top = graph.addPlot({
    color: p$.BOX_COLORS.GRAY.BORDER,
    label: '',
    limit: 1000,
    display: false
  });
  damping_plot_bottom = graph.addPlot({
    color: p$.BOX_COLORS.GRAY.BORDER,
    label: '',
    limit: 1000,
    display: false
...
function setupControls() {
  ...
  controls.amplitude = new p$.Slider({
    id: 'amplitude',
    start: 2,
    min: 0,
    max: 3,
...
function reset() {
  ...
  function calc(t) {
    ...
    var s =
      damp_factor *
      controls.amplitude.value *
      Math.cos(Math.sqrt(omega * omega - y * y) * t);
    path.points.push([t, -s]);
    damping_plot_top.points.push([t, damp_factor * controls.amplitude.value]);
    damping_plot_bottom.points.push([
      t,
      -damp_factor * controls.amplitude.value
    ]);
  }
  ...
  if (controls.damping.value > 0) {
    ...
    do {
      ...
      damping_plot_top.points[damping_plot_top.points.length - 1][1] > 0.05
    );
  } else {
...
function draw() {
  ...
  if (path.points[frame][0] > availableDisplayTime) {
    graph.axis.position.x =
      -(path.points[frame][0] - availableDisplayTime) * graph.scaleX.toPx;
  } else {
...
function drawSpring() {
  ...
  function drawDistances() {
    ...
    if (horizontal_distance_labels) {
      spring.text(
        'Longitud',
        x0 + 0.5,
        natural_y + SPRING_NATURAL_LENGTH / 2 + 0.3
      );
      spring.text(
        'Fija',
        x0 + 0.5,
    ...
    if (horizontal_distance_labels) {
      spring.text(
        'Longitud de',
        x0 + 0.5,
        equilibrium_y + equilibrium_point / 2 + 0.3
      );
      spring.text(
        'Equilibrio',
        x0 + 0.5,
    ...
    if (controls.amplitude.value > 0) {
      ...
      if (horizontal_distance_labels) {
        spring.text(
          'Amplitud',
          x0 + 0.5,
          extended_y + controls.amplitude.value / 2
        );
      } else {
        spring.save();
  ...
  var lastPointX =
    path.points[frame][0] * graph.scaleX.toPx + graph.axis.position.x;
...
function resize() {
  ...
  w.axis.setPosition(
    50,
    CEILING_HEIGHT + SPRING_HANGING_OFFSET * w.scaleX.toPx
  );
