...
var images = {
  rocketOff: new p$.Picture('rocket_off.png', {
    scale: 1,
    isDraggable: false,
    display: false
  }),
  rocketOn: new p$.Picture('rocket.png', {
    scale: 1,
    isDraggable: false,
    display: true
  }),
  soccer: new p$.Picture('soccer.png', {
    scale: 0.5,
    isDraggable: false,
    display: false
  }),
  baseball: new p$.Picture('baseball.png', {
    scale: 0.5,
    isDraggable: false,
    display: false
  }),
...
function setup() {
  ...
  labels.vx = box.addLabel(110, 14, {
    name: 'Vx',
    units: 'm/s',
    labelWidth: 35
  });
  labels.vx.setPosition(0, 25);
  labels.vy = box.addLabel(110, 14, {
    name: 'Vy',
    units: 'm/s',
    labelWidth: 35
  });
  ...
  vel.setZ(1);
  ball.setZ(2);
  images.rocketOn.setZ(3);
  images.rocketOff.setZ(4);
  images.soccer.setZ(5);
  images.baseball.setZ(6);
  box.setZ(10);

  // Add objects to world.
  w.add(ball, vel, box, path, ball_labels);
  w.add(images.rocketOn, images.rocketOff, images.soccer, images.baseball);
}
...
function setupControls() {
  ...
  // Ball type select.
  controls.type = new p$.dom.Select('type', function (val) {
    if (val === 'user') {
      val = 'rocketOff';
    }
    selectedImage = images[val];
    reset();
  });
}
 ...
 */
function reset() {
  // Display the correct projectile.
  images.baseball.display = false;
  images.soccer.display = false;
  images.rocketOff.display = false;
  images.rocketOn.display = false;
  if (images.user !== undefined) images.user.display = false;
  selectedImage.display = true;
...
function drawBallLabels() {
  for (var i = 0; i < steps; i++) {
    ...
    if (b.obj.display && b.obj.position.x !== 0) {
      ball_labels.text(
        'Vy: ' + p$.utils.round(b.vy, 1) + ' m/s',
        b.obj.position.x,
        b.obj.position.y + 8
      );
      ball_labels.text(
        p$.utils.round(b.time, 1) + ' s',
        b.obj.position.x,
        b.obj.position.y + 12
      );
    }
...
function draw() {
  if (started) {
    ...
    if (t < t_final) {
      ...
      ball.setPosition(
        vx0 * t_final,
        y0 + vy0 * t_final - 0.5 * 9.81 * t_final * t_final
      );
      vel.setPosition(ball.position.x, ball.position.y);
  ...
  selectedImage.rotation = (90 - vel.angle()) * p$.DEG_TO_RAD;
  selectedImage.setPosition(ball.position.x, ball.position.y);
}
