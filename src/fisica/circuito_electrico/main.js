...
var RES = [
  // Coordinates for drawing the resistor.
  [LEAD, 0],
  [RX / 2, RY / 2],
  [RX, -RY],
  [RX, RY],
...
function reset() {
  ...
  r_eq =
    controls.r3.value + 1 / (1 / controls.r1.value + 1 / controls.r2.value);
...
function drawCircuit() {
  ...
  if (width > 8) {
    ...
    circuit.save();
    circuit.translate(
      i1_x + 1 * p$.utils.cos(225, p$.ANGLE_STYLE.DEG),
      half_height + 1 * p$.utils.sin(225, p$.ANGLE_STYLE.DEG)
    );
    circuit.rotate(-225);
    circuit.noStroke();
    circuit.fill(p$.COLORS.PURPLE);
  ...
  circuit.equilateralTriangle(
    width - k,
    i1_y,
    TRIANLGE_SIDE + i_ratio - 0.1,
    -90
  );
  ...
  // V1 and V2
  circuit.text(
    p$.utils.round(vr12, 2) + ' V',
    r2_x + TEXT_SEPARATION / 3,
    r1_y - TEXT_SEPARATION / 2
  );
  circuit.text(
    p$.utils.round(vr12, 2) + ' V',
    r1_x + TEXT_SEPARATION / 3,
    r1_y - TEXT_SEPARATION / 2
  );
}
