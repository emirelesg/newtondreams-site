...
function setup() {
  ...
  // Labels
  labels.force = results.addLabel(80, 14, {
    name: 'F',
    units: 'N',
    usePrefixes: true,
    labelWidth: 20,
  ...
  labels.angle = results.addLabel(80, 14, {
    name: p$.SYMBOL.THETA,
    units: '°',
    labelWidth: 20,
    decPlaces: 0
...
function setPosition(component) {
  if (activeQ) {
    var raw = prompt(
      'Posición -' + component + ' de ' + activeQ.lowerLabel + ':',
...
function draw() {
  ...
  if (w.mouse.dragging != p$.DRAG_NOTHING || calculate) {
    ...
    for (var i = 0; i < charges.length; i++) {
      ...
      if (charges[i] !== activeQ && charges[i].display) {
        ...
        vectors.temp_force.setMag(
          Math.round(
            (p$.K * (controls.q[i].value * Q_UNITS) * q_charge) /
              (dx * dx + dy * dy),
            1
          ),
          Math.atan2(dy, dx)
    ...
    results.setPosition(
      activeQ.position.x * w.scaleX.toPx +
        w.axis.position.x -
        results.width / 2,
