...
function setup() {
  ...
  labels.v1 = box.addLabel(110, 14, {
    name: 'V1',
    units: 'm/s',
    labelWidth: 35
  });
  labels.v1.setPosition(0, 25);
  labels.v2 = box.addLabel(110, 14, {
    name: 'V2',
    units: 'm/s',
    labelWidth: 35
  });
...
function drawScene() {
  function drawBlock(block) {
    ...
    s.text(
      controls[block.label].label.val(),
      block.x + block.width / 2,
      block.width > 1 ? block.width / 2 - 0.3 : block.width / 2
    );
    ...
    if (Math.abs(block.vx) > 0) {
      ...
      // Draw the arrow head.
      if (block.vx < 0) {
        s.equilateralTriangle(
          arrowBodyW / 2 - ARROW_HEAD_H / 2,
          0,
          ARROW_HEAD_L,
          180
        );
