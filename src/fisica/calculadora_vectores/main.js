...
var w;
var vectors = [];
var resultant = new p$.Vector({
  componentsAtOrigin: false,
  color: p$.COLORS.GRAY
});
var controls = {};
...
function setupControls() {
  ...
  controls.addVector = new p$.dom.Button('addVector', function () {
    addVector();
    calcResultant();
  });
...
function addVector() {
  ...
  var rowCode = '<tr>';
  rowCode +=
    "<td class='font-weight-bold' style='color: " +
    COLORS[colorIndex] +
    ";'>" +
    letter +
    '</td>';
  ...
  rowCode +=
    "<td><input type='text' class='form-control-sm form-control' placeholder='0'></td>";
  rowCode +=
    "<td><button type='button' class='btn btn-sm toggleComponents btn-block'><i data-feather='eye' class='d-none'></i><i data-feather='eye-off'></i></button></td>";
  rowCode +=
    "<td><button type='button' class='btn btn-sm removeVector btn-danger btn-block'><i data-feather='x'></i></button></td>";
  rowCode += '</tr>';
