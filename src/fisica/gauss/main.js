...
function setup() {
  ...
  labels.qenc = results.addLabel(170, 14, {
    name: 'Qenc',
  ...
  labels.e = results.addLabel(170, 14, {
    name: 'Campo',
    units: 'kN/C',
    labelWidth: 60,
    decPlaces: 0
  ...
  labels.flux = results.addLabel(170, 14, {
    name: 'Flux',
    units: 'kNm²/C',
    labelWidth: 60,
    decPlaces: 0
...
function reset() {
  ...
  var qenc =
    gauss_r < charge_r ? p * ((4 / 3) * p$.PI * Math.pow(gauss_r, 3)) : charge;
  ...
  r_gauss.set(
    radius * p$.utils.cos(45, p$.ANGLE_STYLE.DEG),
    radius * p$.utils.sin(-45, p$.ANGLE_STYLE.DEG)
  );

  // Draw vectors around the sphere.
  var s = 360 / vectors.length;
  for (var i = 0; i < vectors.length; i += 1) {
    vectors[i].setPosition(
      radius * p$.utils.cos(i * s, p$.ANGLE_STYLE.DEG),
      radius * p$.utils.sin(i * s, p$.ANGLE_STYLE.DEG)
    );
...
function drawGauss() {
  ...
  if (radius > 2) {
    ...
    if (e !== 0 && vectors[1].display) {
      ...
      gauss.text(
        'E',
        vectors[1].position.x + vectors[1].x + offset,
        vectors[1].position.y + vectors[1].y + offset
      );
    }
  }
}
