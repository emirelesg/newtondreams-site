...
function setupControls() {
  ...
  controls.graph_type = new p$.dom.Options('graph_type', function () {
    reset();
  });
  ...
  $('input[name="type"]').change(function () {
    $("input[name='type']").each(function () {
      switch ($(this).val()) {
        case 'cylinder':
          cylinder.display = $(this).prop('checked');
          break;
        case 'sphere':
          sphere.display = $(this).prop('checked');
          break;
        case 'wheel':
          wheel.display = $(this).prop('checked');
          break;
      }
    });
    reset();
...
function reset() {
  ...
  for (var i = 0; i < objects.length; i++) {
    ...
    var y =
      controls.graph_type.value === 'pos'
        ? objects[i].d - objects[i].d0
        : controls.graph_type.value === 'vel'
        ? objects[i].v
        : objects[i].accel;
...
function draw() {
  ...
  // Update time labels.
  labels.places.setText(
    cylinder.place_label + '\n' + sphere.place_label + '\n' + wheel.place_label
  );
  labels.times.setText(
    'Cilíndro:   ' +
      cylinder.time_label +
      '\n' +
      'Esfera:     ' +
